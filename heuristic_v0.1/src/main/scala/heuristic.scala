...
object heuristic {
  ...
  def main(args: Array[String])  {
   ...
   // System.setProperty("hadoop.home.dir", "HADOOP_HOME");
    ...
    val input = sc.textFile("file:///media/cocos/Sub Disk/Grad/yellow_tripdata_2015-01.csv").sample(false,max.toDouble/13000000.0,System.currentTimeMillis().toInt)
    ...
    val g_rdd = 
   	result5.filter( x => x._2 > mean).map { line =>
    ...
    println("g_rdd")
	val sort_g = g_rdd.filter(x => x._2 > g_mean).collect.toSeq.sortWith(_._2 >_._2)
    ...
    //val g_map = 
	val writer = new PrintWriter(new File("result_"+args(2)+"_sample.txt"))
...
////////////////////////////##########origin##########/////////////////////
	...
	val sort_g_origin = g_rdd_origin.collect.toSeq.sortWith(_._2 >_._2)
	
	sort_g_origin.take(50).foreach{	line =>
		hotspot += 1;
	 	println(line); 
	}

	val writer_origin = new PrintWriter(new File("result_"+args(2)+"_origin.txt"))

	sort_g_origin.take(50).foreach{
			line => writer.write((line._1._1) + ", " + (line._1._2) + ", " +
			(line._1._3) + " : " + (line._2))
			writer.write("\n")
	}

    val time_finish_origin = java.lang.System.currentTimeMillis();
    val time_origin = time_finish_origin - time_start;

	println("total : " + time_origin);
    writer.write(time.toString);
	writer.close();
