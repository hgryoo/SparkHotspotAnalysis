...
object neighbor{
  def main(args: Array[String]): Unit = {
   ...
   // System.setProperty("hadoop.home.dir", "HADOOP_HOME");
	...
	.registerKryoClasses(Array(neighbor.getClass,classOf[scala.collection.mutable.WrappedArray$ofRef]))
	...
	val g_rdd = sc.parallelize(sort_result5.take(heuristic)).map{ x =>
     	{
	...
	val writer = new PrintWriter(new File(result_path + "/result_"+args(0)+
				"_"+args(1)+"_"+ args(2)+"_"+args(3)+"_neighbor_sampling_v0.1.txt"))
