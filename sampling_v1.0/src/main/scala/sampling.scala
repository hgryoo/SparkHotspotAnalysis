...
object sampling {
  def main(args: Array[String])  {
   ...
   // System.setProperty("hadoop.home.dir", "HADOOP_HOME");
	...
	val g_rdd = sc.parallelize(sort_result5.take(heuristic)).flatMap { line =>
     	{
			var temp = Map[(Int,Int,Int),Double]();
			var i = 0;
			var j = 0;
			var k = 0;
			for (i <- -neighbor_value to neighbor_value){
				for (j <- -neighbor_value to neighbor_value){
					for (k <- -neighbor_value to neighbor_value){
						val x_te = line._1._1 + i ;
						val y_te = line._1._2 + j ;
						val z_te = line._1._3 + k ;

						if (broad_map.value.get(x_te,y_te,z_te) != None)
						{
							val res = find_weight((x_te,y_te,z_te),1)
    ...
    println("g_rdd")
	val sort_g = g_rdd.distinct().collect.toSeq.sortWith(_._2 > _._2)
	...
	val writer = new PrintWriter(new File(args(4) + "/result_"+args(0)+
				"_"+args(1)+"_"+ args(2)+"_sampling_v1.0.txt"))
