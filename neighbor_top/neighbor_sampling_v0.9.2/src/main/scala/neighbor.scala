...
object neighbor{
  ...
  def main(args: Array[String]): Unit = {
   ...
   // System.setProperty("hadoop.home.dir", "HADOOP_HOME");
	...
	var sort_result5= result5.sortBy(_._2,false).zipWithIndex()
	
	val g_rdd = sort_result5.filter(_._2 < (sampling * cube_size).toInt ).map{ line =>
			{
				val x = line._1
	...
	var hotspot = sc.parallelize(sort_g.take(50))
	var hotspot_match = false
	var while_count = 0;

	while ( hotspot_match == false){
		println("count : " + while_count);
		val g_rdd_while = sort_result5.filter(line => line._2 > (while_count * (0.025 * cube_size).toInt) + (sampling * cube_size).toInt - 1 && line._2 < ((while_count + 1) * (0.025 * cube_size).toInt) + (sampling * cube_size).toInt ).map{ line =>
			{
				val x = line._1
				var sum_weight_value = 0.0; //weight x value
				var sum_weight = 0.0;// 
...
//		val union_while = .persist()
		val sort_g_while = g_rdd_while.union(hotspot).collect.toSeq.sortWith(_._2 > _._2)
		val hotspot_while = sc.parallelize(sort_g_while.take(50))

		if (hotspot_while.subtractByKey(hotspot).count() == 0 || (while_count + 1) * sampling > cube_size){
			hotspot_match = true;
			
		}else{
			hotspot = hotspot_while;
			while_count += 1;
		}
	...
	hotspot.take(50).foreach(println(_))
	...
	val writer = new PrintWriter(new File(result_path + "/result_"+args(0)+
				"_"+args(1)+"_"+ args(2)+"_neighbor_sampling_v0.9.txt"))


	hotspot.take(50).foreach{
