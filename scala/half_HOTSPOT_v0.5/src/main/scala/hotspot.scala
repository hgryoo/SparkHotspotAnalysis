...
//	args(5) : column file patha
//	args(6) : sampling_rate
...
//----------------------------------------------------------------------------------------------------//
	...
	val result3 = result_filter.flatMap( line => {
			val temp = new Array[(String,String,String)](key_num);
			for (key <- 0 to key_num -1 ){
				temp(key) = (  line( key_arr(key)(0).toInt ), line( key_arr(key)(1).toInt ), line( key_arr(key)(2).toInt ) );
...
//----------------------------------------------------------------------------------------------------//
	...
	if (sort_g.length < hotspot_num.toInt) hotspot_num = sort_g.length.toString;
