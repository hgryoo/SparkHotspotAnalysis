...
object nonbinary{
  ...
  def main(args: Array[String]): Unit = {
	...
	val distband_value = args(2).toInt
   ...
   // System.setProperty("hadoop.home.dir", "HADOOP_HOME");
	...
	println("data num :" + data_num.value );
	...
	val max_time_index = total_days / time_step
	val g_rdd = sc.parallelize(sort_result5.take(top_amount) , (PART.toDouble * top_percent).toInt).map{ x =>
		{
			...
			var W = 0.0;
			for ( i <- 1 to distband_value){
				var outside = 0;
				//if ( i + x._1._3 > max_time_index){
					//outside = (x._1._3+i-max_time_index) * (2*i+1) * (2*i+1) - (x._1._3+i-max_time_index-1) * (2*(i-1) + 1) * (2*(i-1)+1);				}
				//if ( x._1._3 - i < 0 ){
					//outside = (i - x._1._3) * (2*i+1) * (2*i+1) - (i - x._1._3-1) * (2*(i-1)+1) * (2*(i-1)+1);				}
				
				W += (pow((i * 2 + 1) , 3) - pow( (i-1) * 2 + 1, 3) )* (1 / 1 + i);
			}
			W += 1.0

			for ( i <- -distband_value to distband_value){
				for ( j <- -distband_value to distband_value ) {
					for ( k <- -distband_value to distband_value ) {
						if (x._1._3 + k > -1 && (x._1._3 + k < (210 / time_step) + 1)){
							var max = abs(i)
							if ( max < abs(j) ) { max = abs(j) }
							if ( max < abs(k) ) { max = abs(k) }
						//	val weight = 1.0 / pow(2,max);
							var weight = (1.0 / (max+1) ) / W;
			...
			val denominator = 1000 * sqrt((cube_size * sum_pow_weight - 1)/(cube_size - 1));
			//val molecule = sum_weight_value - mean * sum_weight;
			val molecule = sum_weight_value - mean;
	...
	val writer = new PrintWriter(new File(result_path + "/result_"+args(0)+
				"_"+args(1)+"_"+ args(2)+"_"+args(5)+"_nonbinary_v0.1.0.txt"))
