...
object neighbor{
  ...
  def main(args: Array[String]): Unit = {
   ...
   // System.setProperty("hadoop.home.dir", "HADOOP_HOME");
	...
	val result4 = result3.flatMap( line => {
		val x_or = (line._1.toDouble/degree).toInt
		val y_or = (line._2.toDouble/degree).toInt
		val time = (find_days(line._3.split(' ')(0).split('-')(1).toInt) 
				+ line._3.split(' ')(0).split('-')(2).toInt -1 ) / time_step;
	...
	val pow_sum_mean = (pow_sum.value.toDouble / 27 / 27)  / cube_size.toDouble 
    
	var top_amount = (cube_size * top_percent).toInt
	if (top_amount < 50000) {
		top_amount = 50000;
	}
	val mean = (data_num.value/27).toDouble /cube_size.toDouble; // 
	...
	println("real num :" + data_num.value/27 );
	println("g_rdd")

	val time_finish = java.lang.System.currentTimeMillis();
	val time = time_finish - time_start;

	val writer = new PrintWriter(new File(result_path + "/result_"+args(0)+"_"+args(1)+"_"+ args(4)+"_neighbor_one_v0.0.3.txt"))
