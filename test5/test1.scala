...
object test1 extends App {
 // def main(args: Array[String]) {
    ...
    try{
      ...
      //파일에서 map으로 저장
      breakable{
      for(line <- Source.fromFile("D:/me/Data_about_Major/Grad/30000sample.csv").getLines()){
        num = num + 1;
        real_num = real_num + 1;
        if (num == max+1) break;
        ...
        breakable{
          ...
          var inputData_ID = (inputData_date - 1) / time_step;
      ...
      var mean = (real_num - 1).toDouble / my_map.size.toDouble; // 평균
...
//       }
       ...
       for ((x,y,z) <- sort_g_map.keys){
         hotspot += 1;
         println(x + ", " + y + ", " + z + " : " + sort_g_map(x,y,z));
         if (hotspot == 50) break;
       }
       }
